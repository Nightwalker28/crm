...
def search_quotes(query: str = Query(..., min_length=1, max_length=100), fields: str | None = Query(default=None), sort_by: str | None = Query(default=None), sort_direction: str | None = Query(default=None), filter_logic: str = Query(default="all"), filters: str | None = Query(default=None), filters_all: str | None = Query(default=None), filters_any: str | None = Query(default=None), pagination: Pagination = Depends(get_pagination), db: Session = Depends(get_db), current_user=Depends(require_user), require_module=Depends(require_module_access("sales_quotes")), require_permission=Depends(require_action_access("sales_quotes", "view"))):
    all_conditions, any_conditions = _parse_filters(db, current_user.tenant_id, filter_logic, filters, filters_all, filters_any)

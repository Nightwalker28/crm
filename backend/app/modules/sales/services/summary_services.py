...
def _serialize_io(record: FinanceIO) -> dict:
    return {
        ...
        "customer_name": record.customer_name or record.client_name or record.campaign_name,
...
def _get_related_insertion_orders(
    ...
    if not organization_name and organization_id is None:
        return []
    ...
    if organization_id is not None and normalized is not None:
        filters.append(
            or_(
                ...
                func.lower(func.coalesce(FinanceIO.client_name, "")) == normalized,
                func.lower(func.coalesce(FinanceIO.campaign_name, "")) == normalized,
            )
        )
...
def build_contact_summary(db: Session, contact: SalesContact) -> dict:
    ...
    opportunities = (
        ...
        .filter(SalesOpportunity.contact_id == contact.contact_id)
...
def build_organization_summary(db: Session, organization: SalesOrganization) -> dict:
    contacts = (
        ...
        .filter(SalesContact.organization_id == organization.org_id)
    ...
    opportunities = (
        db.query(SalesOpportunity)
        .filter(SalesOpportunity.organization_id == organization.org_id)
        .order_by(SalesOpportunity.created_time.desc())
        .limit(10)
        .all()
    )

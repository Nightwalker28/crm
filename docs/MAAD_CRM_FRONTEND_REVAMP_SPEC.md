...
- Contract details now use shared semantic cards, fields, status pills, empty states, responsive action layouts, and labeled party and signer controls. Users without the Contracts edit action receive a read-only detail surface; editors explicitly confirm lifecycle changes before the audited update, while party, signer, related-record, and event workflows retain fixed user-facing errors. Existing tenant-scoped contract and linked-record validation, module/action permissions, and event history remain authoritative.
- Insertion Order details now use semantic cards, shared status pills, responsive summaries, linked customer navigation, readable missing values, tenant-defined custom fields, and authenticated imported-file access. Edit visibility follows the Finance IO edit action, while assignee, effective-date, commercial, notes, and activity information remain available to authorized viewers. Existing tenant and user scoping, module/action checks, protected file-path resolution, and fixed detail failures remain authoritative.
...
- Continue the semantic-token and shared-control sweep across residual shared primitives and secondary detail/print surfaces, led by record-activity panels.

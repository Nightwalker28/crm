...
- Dashboard decomposition: module entry points, permission-filtered quick actions, recent activity, notifications, CRM summary and forecast widgets, saved-report charts, Quick Note, searchable Summary Table, per-module summaries, and the complete layout editor now live in semantic-token domain boundaries with scoped loading, empty, retry, and permission states. Dashboard report requests no longer carry backend detail into frontend errors. Note and summary widgets use shared inputs, textarea, and table primitives, and unchanged note blur no longer causes a redundant save. The route-level dashboard dropped from 1,271 to 460 lines with zero legacy neutral or black token occurrences; the extracted layout editor owns its sticky edit toolbar, pointer and keyboard ordering controls, resize and remove controls, empty draft state, and responsive add-widget dialog.
...
## Frontend revamp closure audit

Phase status records describe completed module slices; they do not by themselves prove the project-wide acceptance criteria below.

Verified closed from the earlier gap audit:

- The global palette derives enabled built-in create and workflow actions from the module registry, supports custom-module creation, includes administrator settings navigation, filters actions by module/action permission, records actual user-scoped route visits, and has keyboard, routing, permission, recent-page, payment, report, template, and integration coverage.
- Administrator-only Add user, Create team, Create department, and Create role commands now open addressable create workflows. Their shared registry definitions remain separate from ordinary settings navigation, and non-administrators do not receive them.
- Global record-search failures use fixed recoverable guidance with explicit retry instead of backend response details. Insertion Order create, edit, list, and detail failures also remain on fixed user-facing copy, and the detail route uses the shared loading, recoverable-error, and not-found states.
- Custom-module record creation now uses a permission-gated responsive `/dashboard/custom/[moduleKey]/new` workflow instead of `CustomModuleRecordDialog`. The routed form preserves tenant-configured field order, defaults, required fields, select options, help text, backend value coercion, unsaved-change protection, and safe failure copy; list and palette actions both use the new route.
- Custom-module record lists and details now use shared semantic tables, forms, pagination, loading, filtered-empty, first-use, retry, not-found, and permission-denied states. Create, edit, delete, import, and export controls follow the dynamic module actions granted to the current user; viewers receive a read-only detail surface, destructive actions explain Recycle Bin recovery, and request failures never expose backend details. Existing tenant-scoped dynamic action checks, audited soft deletion, and restoration remain authoritative. Activity, notes, and document linking remain visibly unavailable until shared backend record-reference support covers dynamic module keys.
- Contracts, Products, Services, Insertion Orders, Documents upload, Mail compose, Client Portal page creation, Payments, Message Templates, and Support case creation now use routed workflows where the earlier audit found oversized embedded forms.
- Products, Services, Contracts, Documents, Mail, Client Portal, Insertion Orders, Reports, and Integrations now have explicit Phase 3 or Phase 6 implementation records.
- The Dashboard route is a focused composition surface; its layout shell and add-widget editor use shared controls and semantic tokens without changing user-scoped persistence or permission-filtered widget availability.
- Backups now uses shared semantic tokens, controls, status pills, loading/empty/recoverable-error states, responsive table containment, unsaved-change protection, and destructive confirmation. Backend and stored run failures are defensively redacted while existing tenant-scoped backup, retention, restore, permission, and audit behavior remains unchanged.
- Mail now uses shared cards, inputs, selects, buttons, search, status pills, empty states, and scoped recoverable errors across the inbox, IMAP/SMTP configuration, record linking, and routed composer. Message rows are keyboard-selectable, mailbox fields have accessible labels, disconnection is confirmed, and technical provider or request details remain redacted without changing per-user credentials, provider scopes, tenant-safe linking, or send/sync contracts.
- Client Portal administration now uses shared semantic controls, customer search, status pills, empty states, and scoped recoverable errors across account provisioning, signed pages, client accounts, and routed page creation. Publishing customer-specific pricing and document links, regenerating setup tokens, and changing authenticated client access require explicit confirmation; CRM, client-authenticated, and signed-public boundaries remain separate.
- Documents now uses shared semantic storage metrics, search and filtering, table, status, empty, and scoped recoverable-error patterns across the tenant library and record-linked panels. Template, version, and client-share controls are labeled and shared; sharing and revoking client access require explicit confirmation, and document API failures use fixed user-facing copy without changing tenant access, upload validation, quota, versioning, soft-delete, provider, or client-portal boundaries.
- Module Settings and its department/team access editor now use shared semantic cards, buttons, inputs, selects, tables, status pills, empty states, and recoverable route errors. The table correctly aligns Duplicate Handling and Automation controls with their headers, disabling a tenant module requires consequence-specific confirmation, access changes remain an explicit unsaved draft until saved, and fixed frontend failures preserve the existing administrator-only, tenant-scoped API boundary.
- Customer Groups now uses labeled shared inputs, selects, checkboxes, search, tables, status pills, filtered and dataset empty states, and retryable fixed errors. Name, key, required discount, non-negative value, and percentage-cap validation match the backend before submission; changing the tenant default, deactivating a group, replacing a dirty edit, or clearing a draft requires explicit intent. The admin-only dashboard route and tenant/module/action-scoped API remain unchanged, as do authenticated customer pricing resolution and the separate public catalog boundary.
- Recycle Bin now exposes every backend-supported built-in recovery module, including Calendar, Products, and Services, plus active custom modules. Its selector, table, pagination, loading, empty, custom-module warning, list retry, and restore-failure states use shared semantic controls; restoration requires consequence-specific confirmation, never renders backend detail, refreshes the restored module caches, and corrects out-of-range pagination after the last item is recovered. Existing administrator-only, tenant-scoped lookup, restore, audit, and retention behavior remains authoritative.
- Activity Log now uses shared semantic action filters, refresh, table, status, loading, dataset-empty, filtered-empty, retry, and server-backed pagination patterns. Common actions and entity types render as readable labels, actor IDs distinguish user and system activity, the formerly inert page-size control changes the backend query, and technical failures remain redacted. The UI continues to omit raw before/after state; existing administrator-only and tenant-scoped audit access remains unchanged.
- The Settings landing page now presents every registered settings destination through shared responsive cards and semantic surface, border, copy, focus, and danger-state tokens. Organization, access-control, customization, system, and recovery destinations remain grouped for scanning, all 15 canonical links are represented, and the existing dashboard administrator guard remains authoritative.
- General Settings now uses a labeled, responsive full-page company form with shared semantic cards, controls, status, confirmation, loading, and retry patterns. Required fields and stored column limits are reflected in the browser controls, currency codes normalize before saving, dirty changes are guarded and explicitly discardable, technical failures remain redacted, and an immediately persisted logo no longer creates a false unsaved state. Shared image uploads now enforce a 5 MB read bound while retaining server-side image signature and extension validation; existing administrator writes and tenant-scoped company lookup remain unchanged.
- Shared Import Controls now use semantic menu, dialog, card, field, mapping, summary, progress, and recoverable-error patterns across Leads, Accounts, Contacts, Deals, Quotes, Insertion Orders, and custom modules. File selection enforces the backend-aligned non-empty 50 MB limit before preview, required mappings are labeled and validated in context, overwrite and merge modes require consequence-specific confirmation, progress is accessible and bounded, and preview, execution, and background-job failures do not expose backend details. Existing module/create permissions, tenant scoping, duplicate behavior, and background execution remain authoritative.
- Shared Export Controls now use semantic menu, dialog, scope-selection, summary, progress, and recoverable-error patterns across the same operational module set. POST exports preserve filtered, current-page, and selected-row scopes through accessible radio controls and background jobs; direct GET exports now download CSV responses correctly, repairing custom-module exports that previously expected job JSON and produced no file. Content-Disposition filenames are decoded and reduced to safe basenames, auto-download remains one-shot with an explicit retry, and start, job, and download failures remain redacted. Existing tenant, actor, module, and export-action checks remain authoritative.
- The shared Product and Service detail surface now uses semantic cards, copy, borders, status pills, responsive actions, and accessible media presentation. Edit and soft-delete controls follow each catalog module's granted actions, deletion explains its Recycle Bin recovery path, and the displayed amount is explicitly identified as the public base price while authenticated customer-specific pricing remains separate. Existing tenant, module, action, activity, public-feed, and recoverable-delete boundaries remain authoritative.
- Contracts now use shared semantic list and detail tables, cards, fields, status pills, empty states, responsive actions, keyboard identity links, and labeled party and signer controls. List creation follows the granted action and distinguishes filtered, first-use, and recoverable-error states; users without edit access receive a read-only detail surface, while editors confirm lifecycle changes before the audited update. Existing tenant-scoped contract and linked-record validation, module/action permissions, and event history remain authoritative.
- Insertion Orders now use shared semantic list and detail tables, cards, status pills, responsive summaries, keyboard identity links, linked customer navigation, readable missing values, tenant-defined custom fields, and authenticated imported-file access. Create, import, export, selection, and edit visibility follow the corresponding Finance IO action; list failures remain recoverable and redacted, and owner media is presented with the owner rather than the customer. Existing tenant and user scoping, module/action checks, protected file-path resolution, and fixed detail failures remain authoritative.
- Profile now uses shared semantic cards, fields, status pills, route states, responsive layouts, and a semantic timezone picker. Personal edits are dirty-tracked and guarded, immediately persisted photo uploads do not create false unsaved state, and image selection enforces the backend-aligned non-empty JPG/PNG/WebP 5 MB limit. Profile, photo, and MFA failures use fixed copy; MFA setup clearly presents one-time recovery codes and disabling MFA requires consequence-specific confirmation while backend password and second-factor verification remains authoritative.
- Shared user and team selection now uses semantic surfaces, borders, copy, focus states, responsive popovers, labeled search, explicit selection states, and removable chips across Tasks and Calendar. The shared linked-record picker used for owners and teams across Sales, Contracts, Orders, Quotes, and Support now exposes keyboard-operable combobox/listbox semantics and fixed recoverable errors. Existing tenant-scoped option queries and source-module action checks remain authoritative, and selected task assignments stay visible when an option refresh no longer includes their record.
- Shared tenant custom-field inputs now use semantic surfaces, labeled and described controls, required semantics, numeric stepping, and accessible boolean state across Leads, Contacts, Accounts, Deals, Quotes, and Insertion Orders. Required boolean fields initialize as explicit false values so an untouched valid value is not omitted from the payload, and custom-field definition failures no longer propagate backend detail. Lead record tags now use a semantic keyboard-operable combobox/listbox with removable chips, fixed retryable suggestion errors, and module/action-scoped tenant suggestions.
- Deals, Tasks, and Support now share semantic status and priority presentation through the central status-style contract. Deal stages no longer depend on raw color-family classes, task list, board, and calendar consumers retain one status and priority language, and Support list and detail surfaces no longer duplicate local maps. Unknown values remain readable through the neutral shared fallback instead of being mislabeled as a valid default state.
- Leads, Quotes, and Orders now use the same central semantic status contract instead of page-local lifecycle maps. Lead score grades also share that contract across the operational list and detail score card; Contacted no longer relies on a raw violet color family, and unknown lifecycle or grade values remain readable instead of being presented as a valid default state.
- Active user-administration dialogs now use shared semantic surfaces, controls, focus states, labeled fields, responsive setup-link presentation, fixed mutation errors, guarded async submission, dirty-discard confirmation, and readable MFA state. Create and edit failures do not expose backend detail, self-role changes and self-deactivation are enforced at the tenant-scoped administrator API boundary as well as in the browser, and orphaned approve/reject dialog components from the retired pending-user workflow have been removed.
- Contact regions and Account industries now use neutral shared classification pills instead of assigning raw or status-like colors to descriptive categories. Arbitrary stored values remain readable without substring-based tone guessing, semantic success, warning, danger, and information colors remain reserved for actual state or feedback, and Account list query failures retain fixed user-facing guidance.
- The unused legacy `SavedViewBar` and campaign-specific `filter-button` components have been retired after a repository-wide consumer scan. Active operational lists continue through the shared `SavedViewSelector`, `ModuleListToolbar`, `InlineSavedViewFilters`, and routed view-management system; no current saved-view or filtering behavior changed.
- Shared report charts now use the dashboard's semantic surface, border, copy, grid, cursor, shadow, and chart-color tokens instead of hardcoded neutral and fallback colors. Reports and dashboard report widgets retain the existing shared chart API, formatting, accessible labels, and Recharts behavior.
- Insertion Order customer relationship selection now uses the shared accessible Select primitive instead of the last page-local native control. Switching between Contact and Account retains its existing linked-record and free-text reset behavior, updates the relationship search context, and uses the same labeled keyboard interaction as other routed forms.
- The shared Checkbox primitive now owns a visible semantic default, checked indicator, keyboard focus, disabled treatment, and non-submitting button behavior. Contacts, WhatsApp follow-up, Message Templates, notification webhooks, website API scopes, SSO, and user provisioning no longer use native page-local checkboxes, while existing list, finance, settings, and custom-field consumers inherit the same safe default.
- Deals no longer render unavailable pipeline totals as valid zero values. Summary failures use fixed guidance, keep the separately loaded deal list available, and provide a scoped retry; focused browser coverage verifies redaction and recovery.
- Authenticated client-portal and CRM authentication surfaces now use the shared semantic app, surface, border, copy, feedback, button, input, table, loading, and empty-state language instead of the legacy neutral/red palette. Auth, MFA, client setup, sign-in, support, messages, documents, catalog, bookings, orders, quotes, and signed client pages retain their separate authentication and customer-data boundaries, while network and backend exception text is replaced by fixed recovery guidance.
- Operational quote, order, POS invoice, client-order, signed-client-page, and editable transaction line-item tables now use the shared responsive table primitive. A repository-wide non-print scan has no remaining page-local native tables; printable customer documents remain the intentional fixed-layout exception.
- Palette verification now asserts every accessible built-in and custom-module destination in the registry-backed fixture and exercises Arrow/Enter navigation in addition to shortcut opening, action permissions, administrator filtering, actual route-based recents, routed workflows, and search-error redaction.
- Literal provider-logo colors, tenant-validated client-page accent colors, decorative auth/loading gradients, and the stable theme-independent print palette are intentional branding or generated-document exceptions. Operational controls, state feedback, surfaces, copy, borders, and charts use shared semantic tokens.
- Notifications now use semantic bell, badge, popover, loading, empty, unread, refreshing, and scoped recoverable-error states in the shared center, with matching retry and focus behavior in the dashboard widget. Individual and bulk reads update optimistically with rollback, technical API details remain redacted, and only the single dashboard bridge maintains the realtime subscription instead of every notification consumer. Notification destinations are constrained to internal dashboard routes before link or browser-alert navigation; existing CRM-user authentication plus tenant-and-user-scoped list and read mutations remain authoritative.
- Shared record-activity panels now use one semantic header, loading, empty, recoverable-error, focus, list, and card language across Activity, Notes, Documents, Tasks, and Follow-up. Notes and task forms have explicit labels, mention suggestions expose accessible list semantics, task state is readable without legacy color classes, and all activity, mention, note, task, and follow-up failures use fixed user-facing copy. Existing tenant-record lookup, module view/edit checks, mention-recipient access validation, task assignment rules, authenticated document access, and activity logging remain authoritative across the current Sales, Support, Finance, and Catalog detail consumers.
- POS invoices now share semantic invoice and payment status styles across list and detail views, hide create, edit, delete, and routed form workflows when the corresponding action is not granted, and retain fixed user-facing failures throughout the shared query layer. The printable customer document uses a shared responsive page header, safe accent-color fallback, localized dates, readable status and empty values, semantic line-item and balance structures, and a stable theme-independent print palette. Existing CRM authentication, tenant-scoped invoice lookup, module/action enforcement, soft deletion, linked-customer validation, and company-profile access remain authoritative.
- Public quote proposals now use a responsive semantic document surface with accessible loading, unavailable, temporary-error, retry, content, expiry, and download states. Backend details never reach the page, response shape and download filenames are constrained, public fetches omit CRM credentials and browser caching, and proposal downloads continue when best-effort analytics recording fails. Successful signed-link view and event responses explicitly disable caching, and immediate analytics replays with the same proposal, event, recipient, and hashed request fingerprint are deduplicated. Existing hashed tokens, fixed expiry, deleted-quote exclusion, tenant-match validation, and hashed request metadata remain authoritative.
- Public booking now uses a responsive semantic scheduling surface with a shared timezone control, keyboard-readable slot selection, scoped availability retry, aligned field limits, first-error focus, fixed failure copy, stale-slot recovery, and a confirmation state. Anonymous requests omit CRM credentials and browser caching, successful public responses disable caching, and the booking confirmation no longer exposes internal booking, calendar-event, or CRM-source identifiers. Existing enabled-link resolution, tenant-scoped owner and CRM relationship handling, 31-day slot bounds, busy-time and transaction-level overlap checks, unique slot constraint, and per-link/IP submission limits remain authoritative.
- Product and Service lists now use the shared semantic table language for identity links, stock, pricing, visibility, media, active state, missing values, loading, filtered-empty, first-use, and refreshing states. Create and inline active-state actions follow each catalog module's granted actions, failed mutations retain the prior value and fixed user-facing copy, and catalog queries no longer propagate backend detail text. Existing saved-view search and sorting, server pagination, tenant/module/action enforcement, soft deletion, public-feed rules, and authenticated customer-specific pricing boundaries remain authoritative.
- Teams & Departments now uses shared semantic cards, controls, loading, empty, retry, and scoped mutation-error states. Create and edit dialogs have explicit labels, keyboard submission, addressable palette workflows, and dirty-discard protection; deletion confirms permanent consequences, explains department/team constraints, and never renders backend details. Existing administrator authentication, tenant scoping, department validation, team-user reassignment, and inherited module-permission synchronization remain authoritative.

Still open for acceptance closure:

- Execute the authenticated browser matrix and manual keyboard, mobile completion, focus, and AA contrast checks. Defined Playwright scenarios are not equivalent to executed project-wide proof while the admin MFA credential is unavailable.

...
test("Order creation uses the shared itemized transaction workflow", async ({
  page,
}) => {
  ...
  await expect(
    page.getByText("Select an account or contact for this order."),
  ).toBeVisible();
...
test("Order editing hydrates customer, item, and fulfillment fields", async ({
  page,
}) => {
  ...
  await page.route(`**/sales/orders/${orderId}`, async (route) =>
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        tax_total: "19",
        grand_total: "209",
        delivery_date: "2099-08-01",
        delivery_address: "1 Browser Street",
  ...
  await expect(
    page.getByRole("heading", { name: "Edit SO-BROWSER-1" }),
  ).toBeVisible();
  await expect(page.getByText(/Last modified/)).toBeVisible();
  await expect(page.getByPlaceholder("Search accounts")).toHaveValue(
    "Acme Operations",
  );

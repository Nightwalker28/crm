...
test("Quote editing hydrates the routed itemized workflow", async ({ page }) => {
  const quoteId = 987654341;
  await page.route(`**/sales/quotes/${quoteId}/summary`, async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({
    quote: { quote_id: quoteId, quote_number: "Q-BROWSER-1", title: "Browser proposal", customer_name: "Browser Customer", contact_id: null, organization_id: null, opportunity_id: null, assigned_to: null, status: "draft", issue_date: "2099-07-01", expiry_date: "2099-07-31", currency: "USD", notes: "Review terms", updated_at: "2099-07-10T10:00:00Z", custom_fields: {}, items: [{ name: "Implementation", description: "Configured delivery", quantity: "2", unit_price: "100", discount_amount: "10", tax_amount: "19" }] },
    contact: null, organization: null, opportunity: null,
  }) }));

  await page.goto(`/dashboard/sales/quotes/${quoteId}/edit`);
  await expect(page.getByRole("heading", { name: "Edit Q-BROWSER-1" })).toBeVisible();
  await expect(page.getByLabel("Customer name")).toHaveValue("Browser Customer");

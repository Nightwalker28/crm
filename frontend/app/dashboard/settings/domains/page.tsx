      ...
      <Card className="px-4 py-4">
        <div className="flex flex-col gap-4">
          ...
          {settings.isLoading ? <p aria-live="polite" className="text-sm text-copy-muted">Loading custom domains…</p> : settings.tenantDomains.length ? <div className="grid gap-3 xl:grid-cols-2">{settings.tenantDomains.map((domain) => (
            <section key={domain.id} className="flex min-w-0 flex-col gap-4 rounded-control border border-line-default bg-surface-muted p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h3 className="break-all font-mono text-sm font-semibold">{domain.hostname}</h3><div className="mt-2 flex gap-2"><span className={domain.status === "verified" ? "rounded-control bg-state-success-subtle px-2 py-0.5 text-xs text-state-success" : domain.status === "failed" ? "rounded-control bg-state-danger-subtle px-2 py-0.5 text-xs text-state-danger" : "rounded-control bg-state-warning-subtle px-2 py-0.5 text-xs text-state-warning"}>{domain.status.charAt(0).toUpperCase() + domain.status.slice(1)}</span>{domain.is_primary ? <span className="rounded-control bg-surface-hover px-2 py-0.5 text-xs">Primary</span> : null}</div></div><div className="flex gap-2"><Button variant="secondary" onClick={() => void verify(domain.id)} disabled={settings.isSaving || domain.status === "verified"}>Verify</Button><Button variant="ghost" size="icon-sm" aria-label={`Remove ${domain.hostname}`} onClick={() => setDeleting({ id: domain.id, hostname: domain.hostname })}><Trash2 className="size-4" /></Button></div></div>
              <dl className="grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs uppercase text-copy-muted">Record type</dt><dd className="mt-1 font-mono">TXT</dd></div><div><dt className="text-xs uppercase text-copy-muted">Last checked</dt><dd className="mt-1 text-copy-secondary">{domain.last_checked_at ? formatDateTime(domain.last_checked_at) : "Never"}</dd></div><DnsValue label="Host / name" value={domain.txt_record_name} onCopy={() => void copy(domain.txt_record_name, "DNS host")} /><DnsValue label="Expected value" value={domain.txt_record_value ?? ""} onCopy={() => void copy(domain.txt_record_value, "DNS value")} /></dl>

...
export default function QuotesTable({
  ...
  function renderCell(quote: Quote, column: string) {
    if (isCustomFieldColumnKey(column))
      return <CustomFieldCell column={column} values={quote.custom_fields} />;
    switch (column) {
      case "quote_number":
        return (
        ...
        return (
          <TableCell>
            <span className="text-sm text-copy-muted">
              {quote[column as keyof Quote]
                ? formatDateOnly(String(quote[column as keyof Quote]))
                : "-"}
            </span>
          </TableCell>
        );
        ...
        return (
          <TableCell>
            <span className="text-sm tabular-nums text-copy-primary">
              {formatMoney(
                quote[column as keyof Quote] as string | number | null,
                quote.currency,
              )}
            </span>
          </TableCell>
        ...
        return (
          <TableCell>
            <span className="text-sm text-copy-secondary">
              {String(quote[column as keyof Quote] ?? "") || (
        ...
        <TableHeader>
          <TableHeaderRow>
            <TableHead className="w-12 pr-0">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  onToggleCurrentPage?.(checked === true)
                }
                className="h-4 w-4 rounded border border-line-strong bg-surface-raised"
                aria-label="Select current page quotes"
              >
            ...
            {visibleColumns.map((column) => {
              ...
              const sortable =
                !isCustomFieldColumnKey(column) && SORTABLE_COLUMNS.has(column);
              return sortable ? (
                <SortableHead
                  key={column}
                  ...
                  className={
                    column === "quote_number"
                      ? "sticky left-12 z-20 bg-surface"
                      : undefined
                  }
                >
                  {label}
          ...
          {isLoading ? (
                ...
                <EmptyState
                  ...
                  description={
                    hasActiveFilters
                      ...
                      : "Create a quote or import existing quotes from CSV."
                  }
                  action={
            ...
            quotes.map((quote) => (
              <TableRow key={quote.quote_id}>
                ...
                >
                  <Checkbox
                    checked={selectedIds.includes(quote.quote_id)}
                    onCheckedChange={(checked) =>
                      onToggleRow?.(quote.quote_id, checked === true)
                    }
                    className="h-4 w-4 rounded border border-line-strong bg-surface-raised"
                ...
                {visibleColumns.map((column) => (
                  <Fragment key={column}>{renderCell(quote, column)}</Fragment>
                ))}
              </TableRow>
            ))
          )}

      ...
      .map((assignee) => {
        if (assignee.assignee_type === "user" && assignee.user_id) {
          ...
          return {
            ...
            id: user.id,
            label: user.name,
            subtitle: user.team_name ? `${user.team_name} · ${user.email || "No email"}` : user.email || "No email",
          };
        }
        ...
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">
              {selectedEntries.length ? `${selectedEntries.length} assignee${selectedEntries.length === 1 ? "" : "s"} selected` : "Search users or teams"}
            </span>
          ...
          <div className="max-h-80 overflow-y-auto p-2 custom-scrollbar">
            {!normalizedQuery ? (
              <div className="px-3 py-8 text-center text-sm text-neutral-500">
                Start typing to find users or teams.
              </div>
            ) : !filteredUsers.length && !filteredTeams.length ? (
              <div className="px-3 py-8 text-center text-sm text-neutral-500">
                No assignees matched that search.
              </div>
            ) : (
              <div className="space-y-3">
                {filteredUsers.length ? (
                  <div className="overflow-hidden rounded-xl border border-white/6 bg-white/[0.02] p-1">
                    ...
                    {filteredUsers.map((user) => {
                      const selected = hasAssignee(value, "user", user.id);
                      return (
                        <button
                          key={user.id}
                          type="button"
                          onClick={() => onChange(toggleAssignee(value, "user", user.id))}
                ...
                {filteredTeams.length ? (
                  <div className="overflow-hidden rounded-xl border border-white/6 bg-white/[0.02] p-1">
                    ...
                    {filteredTeams.map((team) => {
                      const selected = hasAssignee(value, "team", team.id);
                      return (
                        <button
                          key={team.id}
                          type="button"
                          onClick={() => onChange(toggleAssignee(value, "team", team.id))}
                          className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm text-neutral-200 transition-colors hover:bg-white/8"
                        >
                          <div className="min-w-0">
                            <div className="truncate font-medium text-neutral-100">{team.name}</div>
                            <div className="truncate text-xs text-neutral-500">Team assignment notifies current team members.</div>
                          </div>
                          {selected ? <Check className="ml-3 h-4 w-4 shrink-0 text-white" /> : null}
                        </button>
                      );
      ...
      {selectedEntries.length ? (
        <div className="flex flex-wrap gap-2">
          {selectedEntries.map((entry) => (
            <div
              key={entry.key}
              className="inline-flex items-center gap-2 rounded-full border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-xs text-neutral-200"
            >
              <span className="font-medium">{entry.label}</span>
              <span className="text-neutral-500">{entry.type === "user" ? "User" : "Team"}</span>
              <button
                type="button"
                disabled={disabled}
                onClick={() => onChange(toggleAssignee(value, entry.type, entry.id))}
        ...
        <div className="text-sm text-neutral-500">No assignees selected.</div>
      )}
    </div>
  );
}

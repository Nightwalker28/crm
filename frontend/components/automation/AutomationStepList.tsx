...
function StepRow({ icon: Icon, eyebrow, title, description, selected, onClick, actions, testId }: {
  ...
  return <div data-testid={testId} className={cn("flex items-center gap-3 rounded-[var(--radius-card)] border border-line-default bg-surface p-3", selected && "border-primary bg-action-primary-muted")}>
    <button type="button" onClick={onClick} className="flex min-w-0 flex-1 items-center gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
      ...
      <span className="min-w-0"><span className="block text-[11px] font-semibold uppercase tracking-wide text-copy-muted">{eyebrow}</span><span className="mt-0.5 block truncate text-sm font-semibold text-copy-primary">{title}</span><span className="mt-0.5 block truncate text-xs text-copy-muted">{description}</span></span>
    </button>

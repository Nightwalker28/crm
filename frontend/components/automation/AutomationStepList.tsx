...
export function AutomationStepList({ draft, selection, triggerLabel, triggerDescription, conditionFields, actionDefinitions, messages, onInspect, onAddCondition, onRemoveCondition, onAddAction, onRemoveAction, onMoveAction }: {
  ...
  return <div className="mx-auto grid max-w-2xl gap-3">
    ...
    <button type="button" onClick={() => onInspect({ kind: "validation" })} className={cn("flex items-center justify-between gap-3 rounded-[var(--radius-card)] border border-line-default bg-surface px-4 py-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus", selection.kind === "validation" && "border-primary bg-action-primary-muted")}><span className="flex items-center gap-3">{messages.length ? <CircleDot className="h-5 w-5 text-state-warning" /> : <CheckCircle2 className="h-5 w-5 text-state-success" />}<span><span className="block text-sm font-semibold text-copy-primary">Validation and status</span><span className="block text-xs text-copy-muted">{messages.length ? `${messages.length} item${messages.length === 1 ? "" : "s"} need attention` : "Builder fields look complete"}</span></span></span><ChevronRight className="h-4 w-4 text-copy-disabled" /></button>
  </div>;
}
